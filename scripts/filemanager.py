...
class FileManager(QMainWindow):
    ...
    def apply_theme(self):
        ...
        # Semi-transparent panels with better visibility
        bg_rgba = f"rgba({bg.red()},{bg.green()},{bg.blue()},80)"  # More transparent
        hover_rgba = f"rgba({hover.red()},{hover.green()},{hover.blue()},60)"
        accent_rgba = f"rgba({accent.red()},{accent.green()},{accent.blue()},100)"
        header_rgba = f"rgba({hover.red()},{hover.green()},{hover.blue()},80)"  # Match panel transparency
    ...
    def show_context_menu(self, position):
        ...
        elif action == rename_action and rename_action: self.rename_file()
    ...
    def rename_file(self):
        ...
        old_path = Path(selected[0].data(Qt.UserRole))
    ...
    def delete_files(self):
        ...
        if not paths: return
        reply=QMessageBox.question(self,"Move to Trash",f"Move {len(paths)} item(s) to trash?")
        if reply!=QMessageBox.Yes: return
        errors=[]
        for p in paths:
            try:
                # Use trash-put to safely move to trash
                result = subprocess.run(['trash-put', p], capture_output=True, text=True)
                if result.returncode != 0:
                    errors.append(f"{p}: {result.stderr}")
            except FileNotFoundError:
                # Fallback to permanent delete if trash-put not found
                try:
                    path=Path(p)
        ...
        self.refresh()
        if errors: QMessageBox.warning(self,"Errors","\n".join(errors))
        else: self.statusBar().showMessage(f"Moved {len(paths)} item(s) to trash")
