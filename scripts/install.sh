...
sudo -u "$USER_NAME" yay -S --noconfirm python-pywal16 python-pywalfox vscodium-bin\
    > /tmp/hypr_install_log 2>&1 &
spinner "$!" "Installing Pywal16, Pywalfox & VSCodium from AUR"
wait $! || print_error "Failed to install AUR packages (see /tmp/hypr_install_log)"
print_success "Pywal16, Pywalfox & VSCodium installed"
...
systemctl enable NetworkManager.service 2>/dev/null && print_success "NetworkManager enabled"     || true
